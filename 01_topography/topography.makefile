...
topojson: merge
	...
	$(TOPOJSON_LOC) --id-property none -q $(QUANTIZATION) --simplify-proportion=0.5 -o shades.topo.json -- shade_50=shade.1.tmp.shp shade_100=shade.2.tmp.shp shade_150=shade.3.tmp.shp shade_200=shade.4.tmp.shp
...
polygonize_slices: raster_slice zvals
	...
	for i in 1 2 3 4; do \
		gdal_polygonize.py shades.$${i}.tmp.tif -f "ESRI Shapefile" shade.$$i.tmp.shp shade_$${i} shade ;\
	done

raster_slice: crop zvals
	Slices=( $$(cat ./slices.tmp.txt) ); \
	for i in "$${Slices[@]}"; do \
		gdal_calc.py -A crop_xl.tmp.tif --outfile=level$$i.tmp.tif --calc="$$i*(A>$$i)" 	--NoDataValue=0;\
	done

	gdal_calc.py -A ./shadedrelief.tmp.tif --outfile=shades.1.tmp.tif --calc="50*(A<50)"   --NoDataValue=0
	gdal_calc.py -A ./shadedrelief.tmp.tif --outfile=shades.2.tmp.tif --calc="100*(A<100)" --NoDataValue=0
	gdal_calc.py -A ./shadedrelief.tmp.tif --outfile=shades.3.tmp.tif --calc="150*(A<150)" --NoDataValue=0
	gdal_calc.py -A ./shadedrelief.tmp.tif --outfile=shades.4.tmp.tif --calc="200*(A<200)" --NoDataValue=0
